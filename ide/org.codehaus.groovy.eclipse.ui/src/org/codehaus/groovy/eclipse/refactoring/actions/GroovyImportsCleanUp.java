...
public class GroovyImportsCleanUp extends AbstractGroovyCleanUp {
    ...
    public ICleanUpFix createFix(CleanUpContext context) throws CoreException {
        ...
        if (hasAmbiguity[0]) {
            status.addInfo(MessageFormat.format(ActionMessages.OrganizeImportsAction_multi_error_unresolvable, getLocationString(unit)));
        } else if (edit == null) {
            status.addInfo(MessageFormat.format(ActionMessages.OrganizeImportsAction_multi_error_parse, getLocationString(unit)));
        }
